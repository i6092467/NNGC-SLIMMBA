"""
Runs leave-one-subject-out cross-validation for predicting sleep stages based on time course mass spectrometry data.
"""
import numpy as np
...
def load_data():
    ...
    for i in range(0, n_patients):
        ...
        # Reverse the time series (it seems that the brain governs the molecules)
        if reverse:
...
def construct_lagged_dataset(x_list, y_list, y_list_one_hot, lag, auto_regress=True, ignored_subjects=[]):
    ...
    print("We have " + str(big_x.shape[0]) + " instances")
    print("---------------------------------------------")
...
def get_causal_weights_mlp(x_list, y_list, y_list_one_hot, lag, h_size_1, h_size_2, l_rate, num_epochs, lmbd, alpha=0.8,
    ...
    num_vars = n_molecules + 1 # n_sleep_stages
...
def run_gc_analysis():
    ...
    # Number of predictors
    num_vars = n_molecules  # n_sleep_stages
    ...
    for p in range(n_patients):
        # Construct a dataset with lagged instances
        big_x, big_y, rep_labels = construct_lagged_dataset(x_list, y_list, y_list_one_hot, lag=lag,
                                                              ignored_subjects=[p], auto_regress=False)
        # Construct a dataset with lagged instances
        test_x, test_y, garbage = construct_lagged_dataset([x_list[p]], [y_list[p]], [y_list_one_hot[p]], lag=lag,
                                                              ignored_subjects=ignored_subjects, auto_regress=False)
        model = mlpgc.MLPgc(num_vars=num_vars, device=device, lag=lag, hidden_size_1=n_hidden_units_1,
                            hidden_size_2=n_hidden_units_2, num_outputs=num_outputs)
        ...
        for e in range(n_epochs):
            ...
            for i in range(0, big_x.shape[0], mini_batch_size):
                ...
                l1_regularization = torch.norm(model.imp_weights, 1)
                l2_regularization = torch.norm(model.imp_weights, 2)
                loss = base_loss + lmb * (alpha * l1_regularization + (1 - alpha) * l2_regularization)
        ...
        cm_vldn = np.array([[0, 0], [0, 0]])
        model.eval()
        with torch.no_grad():
            for l in range(0, test_x.shape[0], mini_batch_size):
                # Transform to tensors for PyTorch
                inputs_vldn = Variable(torch.tensor(test_x[l:np.min([l + mini_batch_size, test_x.shape[0]]), :],
                                                    dtype=torch.float)).float().to(device)
                targets_vldn = Variable(torch.tensor(test_y[l:np.min([l + mini_batch_size, test_x.shape[0]])],
                                                     dtype=torch.long)).long().to(device)
                # Compute outputs
                outputs_vldn = model(inputs_vldn)

                cm_l = sklearn.metrics.confusion_matrix(y_true=targets_vldn.data.cpu(),
                                                        y_pred=np.argmax(outputs_vldn.data.cpu(), 1),
                                                        labels=[0, 1])
                cm_vldn = cm_vldn + cm_l
